...
def test_centered_orthogonalized_bank_preserves_shape_and_identity_geometry() -> None:
    raw = (
        (0.2, -0.7, 1.4),
        (1.5, 0.3, -0.2),
        (-0.9, 1.1, 0.8),
    ...
    candidates = centered_orthogonalized_candidate_bank(raw)
    diagnostics = controlled_design_diagnostics(candidates)

    assert len(candidates) == len(raw)
    assert all(len(row) == 3 for row in candidates)
    assert diagnostics.maximum_absolute_feature_mean < 1e-12
    assert diagnostics.maximum_absolute_gram_error < 1e-12


def test_controlled_banks_reject_unidentifiable_shape() -> None:
    ...
    with pytest.raises(ValueError):
        centered_orthogonalized_candidate_bank(((1.0, 2.0), (2.0, 4.0), (3.0, 6.0)))
    with pytest.raises(ValueError):
        centered_orthogonalized_candidate_bank(((1.0, 2.0), (3.0,)))
