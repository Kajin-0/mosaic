...
from .science_s1_v5_simulation import run_ground_truth_simulation_v5

BENCHMARK_VERSION = "s1-population-ranking-benchmark-v5a"
...
def _ranking_summary(
    ...
    if not metrics:
        raise ValueError("ranking metrics must not be empty")
    return {
...
def run_benchmark_v5a(
    *,
    ...
    if min(candidate_count, decision_count, reference_difference_count, heldout_count) <= 0:
        raise ValueError("all synthetic bank counts must be positive")
    ...
    for feature_dimension in dimensions:
        ...
        # For independent standard-normal future candidates X_a,X_b,
        # Delta X = X_a-X_b is exactly Normal(0, 2 I).
        ...
        for query_count in budgets:
            for policy in policy_names:
                ...
                for seed in seed_values:
                    ...
                    result = run_ground_truth_simulation_v5(
                        policy=policy,
                    ...
                    acquisition_scores = result.selected_acquisition_scores
                    raw_runs.append(
                        {
                            "feature_dimension": feature_dimension,
                            "query_count": query_count,
                            ...
                            "acquisition_score_mean": (
                                mean(acquisition_scores) if acquisition_scores else None
                            ),
                            "acquisition_score_min": (
                                min(acquisition_scores) if acquisition_scores else None
                            ),
                            "acquisition_score_max": (
                                max(acquisition_scores) if acquisition_scores else None
                            ),
                            "negative_acquisition_score_count": sum(
                                score < -1e-12 for score in acquisition_scores
                            ),
    ...
    return {
        ...
        "scientific_scope": (
            "Exploratory synthetic population-ranking acquisition benchmark under the correctly "
        ...
        "config": {
            ...
            "reference_differences_are_separate_from_heldout": True,
...
def main() -> None:
    print(json.dumps(run_benchmark_v5a(), indent=2, sort_keys=True))
