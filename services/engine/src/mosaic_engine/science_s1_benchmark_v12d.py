...
BENCHMARK_VERSION = "s1-transverse-stopping-benchmark-v12d"
STOPPING_VERSION = "laplace-transverse-debiased-angular-v4"
SEEDS = tuple(range(320, 448))
RULES = (
    "raw_two_consecutive",
    "radial_debiased_two_consecutive",
    "transverse_debiased_single",
    "transverse_debiased_two_consecutive",
)
PRIMARY_RULE = "transverse_debiased_two_consecutive"
...
def _run_path(
    *,
    ...
    for round_index in range(candidate_count - 1):
        ...
        transverse_risk = posterior_transverse_debiased_directional_risk(
            posterior,
            quantile=posterior_quantile,
            sample_count=posterior_samples,
            seed=risk_seed,
        ...
        checkpoints.append(
            {
                ...
                "transverse_debiased_directional_risk": asdict(transverse_risk),
...
def _risk_key(rule: str) -> str:
    ...
    if rule == "radial_debiased_two_consecutive":
        return "radial_debiased_directional_risk"
    if rule in {"transverse_debiased_single", "transverse_debiased_two_consecutive"}:
        return "transverse_debiased_directional_risk"
    raise ValueError(f"unknown rule: {rule}")


def _required_streak(rule: str) -> int:
    if rule == "transverse_debiased_single":
        return 1
    if rule in {
        "raw_two_consecutive",
        "radial_debiased_two_consecutive",
...
def _fixed_checkpoint_diagnostics(
    ...
    for index, first_checkpoint in enumerate(first):
        raw_covered = 0
        radial_covered = 0
        transverse_covered = 0
        transverse_retained: list[float] = []
        for path in paths:
            checkpoints = cast(list[dict[str, object]], path["checkpoints"])
            ...
            radial = cast(dict[str, object], checkpoint["radial_debiased_directional_risk"])
            transverse = cast(
                dict[str, object],
            ...
            radial_covered += truth <= float(cast(float, radial["upper_error"]))
            transverse_covered += truth <= float(cast(float, transverse["upper_error"]))
            transverse_retained.append(float(cast(float, signal["retained_fraction"])))
        summaries.append(
            {
                ...
                "transverse_debiased_q95_coverage": transverse_covered / len(paths),
                "mean_transverse_retained_fraction": mean(transverse_retained),
            }
        )
    return summaries
...
def run_benchmark_v12d(
    *,
    ...
    if not seed_values or any(seed < 320 for seed in seed_values):
        raise ValueError("v12d seeds must be disjoint from v12a-v12c seeds 0-319")
    ...
    return {
        ...
        "scientific_scope": (
            "Prospective fresh-seed test of transverse-only covariance debiasing for S1 "
            "directional stopping. The correction subtracts tr(Sigma_beta)-u.T Sigma_beta u "
            "from squared fitted slope norm, leaving longitudinal covariance unpenalized. "
...
def main() -> None:
    print(json.dumps(run_benchmark_v12d(), indent=2, sort_keys=True))
