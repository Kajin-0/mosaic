...
BENCHMARK_VERSION = "s1-cone-cover-benchmark-v13j"
METHOD_VERSION = "prequential-cone-cover-nested-confidence-v1"
...
SEEDS = tuple(range(704, 832))
BASELINE = "mixture_all"
CONE_COVER = "cone_cover"
COVER_OFFSETS_DEGREES = (-150, -120, -90, -60, -30, 30, 60, 90, 120, 150, 180)
...
def _cover_indices(index: int, parameter_count: int) -> tuple[int, ...]:
    if parameter_count != 72:
        raise ValueError("v13j cone-cover indices require the 72-point 5-degree grid")
    offsets = tuple(int(round(value / GRID_SPACING_DEGREES)) for value in COVER_OFFSETS_DEGREES)
    return tuple((index + offset) % parameter_count for offset in offsets)


def _cone_cover_log_joint_numerators(log_likelihoods: Sequence[float]) -> tuple[float, ...]:
    count = len(log_likelihoods)
    log_support = log(len(COVER_OFFSETS_DEGREES))
    return tuple(
        _logsumexp(tuple(log_likelihoods[k] for k in _cover_indices(j, count))) - log_support
        for j in range(count)
    )


def _cone_cover_current_set(
    log_likelihoods: Sequence[float], *, alpha_level: float
) -> tuple[int, ...]:
    if not 0.0 < alpha_level < 1.0:
        raise ValueError("alpha_level must lie strictly between zero and one")
    log_q = _cone_cover_log_joint_numerators(log_likelihoods)
...
def _run_path(
    *,
    ...
    current_sets = {
        ...
        CONE_COVER: tuple(range(len(parameters))),
    }
    nested_sets = dict(current_sets)
    first_stop: dict[str, dict[str, object] | None] = {BASELINE: None, CONE_COVER: None}
    first_truth_exclusion: dict[str, int | None] = {BASELINE: None, CONE_COVER: None}
    first_nested_empty: dict[str, int | None] = {BASELINE: None, CONE_COVER: None}

    for observation_index in range(1, max_observations + 1):
        features = _select_disagreement_feature(current_sets[BASELINE], parameters, candidates)
        baseline_probability = _mixture_predictive_probability(
            log_likelihoods, parameters, features
        )
        ...
        baseline_current = _confidence_indices(
            baseline_log_q, log_likelihoods, alpha_level=alpha_level
        )
        ...
        all_current = {BASELINE: baseline_current, CONE_COVER: cover_current}
        ...
        for name in (BASELINE, CONE_COVER):
...
def _summarize(
    paths: Sequence[dict[str, object]], *, predictor: str, horizon: int
) -> dict[str, object]:
...
def _paired_at_horizon(paths: Sequence[dict[str, object]], *, horizon: int) -> dict[str, object]:
    ...
    return {
        ...
        "left": CONE_COVER,
        "right": BASELINE,
        "left_only": cover_only,
        "right_only": baseline_only,
        "both": both,
        "neither": neither,
        "paired_exact_p_value": _paired_exact_p_value(cover_only, baseline_only),
    }


def run_benchmark_v13j(
    *,
        ...
        for seed in seed_values:
            paths.append(
                _run_path(
                    ...
                    seed=100_000_000 + true_angle_index * 100_000 + seed,
    ...
    return {
        ...
        "scientific_scope": (
            "Fresh-seed test of a candidate-specific sparse mixture covering the full region "
            "outside the 27-degree certification cone at 30-degree angular spacing. The fixed "
            "11-point support is chosen from geometry before observing benchmark responses; "
            "acquisition remains frozen to the historical current-time all-grid-mixture controller."
        ),
        "config": {
            ...
            "alpha_level": alpha_level,
            "baseline": BASELINE,
            "candidate": CONE_COVER,
            "cover_offsets_degrees": COVER_OFFSETS_DEGREES,
        ...
        "summaries": [
            _summarize(paths, predictor=name, horizon=horizon)
            for name in (BASELINE, CONE_COVER)
...
def main() -> None:
    print(json.dumps(run_benchmark_v13j(), indent=2, sort_keys=True))
